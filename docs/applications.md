# Applications & Use Cases

:::info Illustrative directions
These examples illustrate where the SENEX design is headed. They combine shipped capabilities (local-first processing, governed sharing) with roadmap ones (MPC aggregation, network-scale GENOME). Compliance references are design goals, not certifications.
:::

## Pull-Based Contextual Services

### Navigation Example
1. User requests city navigation through [AIA agent](architecture/aia_agents.md)
2. Contributing clients share encrypted location/destination data
3. [Genome](architecture/genome.md) processes aggregated traffic patterns via MPC
4. Optimized routes returned with privacy preservation
5. Contributors rewarded based on data utility

### Healthcare Predictions
- Local health data processing (HIPAA compliant)
- Encrypted symptom pattern sharing
- Population-level disease prediction
- Individual risk assessment without data exposure

### Financial Advisory
- Personal spending analysis (local-only)
- Market trend aggregation (anonymized)
- Investment recommendations with privacy guarantees
